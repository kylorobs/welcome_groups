import { InformationCircleIcon, HeartIcon, VideoCameraIcon } from '@heroicons/react/20/solid';

const SecondaryCard = ({ title, imageUrl, date, id, favourited, favouriteClick, infoClick, sessionUrl }) => {
    const favouriteButton = (
        ...
        >
            <HeartIcon className={`h-10 w-10 ${favourited ? 'text-[#fa7185]' : 'text-gray-400'}`} aria-hidden="true" />
    ...
    const watchButton = (
        <a
            ...
            rel="noreferrer"
        >
            <VideoCameraIcon className="h-10 w-10 text-[#f79f24]" aria-hidden="true" />
            <span className="ml-3">Attend Session</span>
        </a>
    );
        ...
        <li key={id} className="col-span-1 divide-y divide-gray-200 rounded-lg bg-white shadow my-8">
            ...
            <div>
                <div className="-mt-px flex divide-x divide-gray-200">
                    ...
                    <div className="-ml-px flex w-0 flex-1">{sessionUrl ? watchButton : favouriteButton}</div>
                </div>
            </div>
        </li>
    );
